...
/**
 * Stateful collector that makes use of {@link LineMerger} to merge multiple {@link LineString}s. If all line strings can be combined into a
 * single LineString then the result will be a list with a single result. If there are gaps in the line string and the result cannot map to
 * a single line string, the {@link Optional#empty()} is returned.
 */
@Slf4j
